...
class TrainKey(object):
    def __init__(self, train_rare_input, sentence_input, output_predict):
        ...
        self.unaryRule = {}     #count(x-w)
        self.unaryMatch = {}
        self.binaryRule = {}    #count(x->yz)
        self.binaryMatch = {}
            ...
            for line in fp:
                if line.strip():
                    ...
                    if parts[1] == 'NONTERMINAL':
                        ...
                        self.unaryRule[parts[2],parts[3]] = self.unaryRule.get((parts[2], parts[3]),0) + int(parts[0])
                        ...
                        self.binaryRule[parts[2], parts[3], parts[4]] = self.binaryRule.get((parts[2], parts[3], parts[4]), 0) + int(parts[0])
    ...
    def get_rule_prob(self):
        #calculate binaryRule Prob
        for key, val in self.binaryRule.items():
            countX = self.nonterminal.get(key[0])
            prob = float(val)/countX
            self.rule_pro[key] = prob
        #calculate UnaryRule Prob
        for key1, val1 in self.unaryRule.items():
            countX1 = self.nonterminal.get(key1[0])
            prob1 = float(val1)/countX1
            self.rule_pro[key1] = prob1
